...
//     return output
    ...
    function applyTheta(state) {
        ...
        // Step 2: Compute D[x]
        for (let x = 0; x < 5; x++) {
            D[x] = C[(x + 4) % 5] ^ ((C[(x + 1) % 5] << 1n) | (C[(x + 1) % 5] >> window.BigInt(w - 1))); // Left rotation by 1, using BigInt
        }
    ...
    function applyPi(state) {
        const newState = Array(5).fill(0).map(() => Array(5).fill(window.BigInt(0))); // Initialize a new 5x5 state with zeros
    ...
    const ROUND_CONSTANTS = [
        window.BigInt(0x0000000000000001), window.BigInt(0x0000000000008082), window.BigInt(0x800000000000808A),
        window.BigInt(0x8000000080008000), window.BigInt(0x000000000000808B), window.BigInt(0x0000000080000001),
        window.BigInt(0x8000000080008081), window.BigInt(0x8000000000008009), window.BigInt(0x000000000000008A),
        window.BigInt(0x0000000000000088), window.BigInt(0x0000000080008009), window.BigInt(0x000000008000000A),
        window.BigInt(0x000000008000808B), window.BigInt(0x800000000000008B), window.BigInt(0x8000000000008089),
        window.BigInt(0x8000000000008003), window.BigInt(0x8000000000008002), window.BigInt(0x8000000000000080),
        window.BigInt(0x000000000000800A), window.BigInt(0x800000008000000A), window.BigInt(0x8000000080008081),
        window.BigInt(0x8000000000008080), window.BigInt(0x0000000080000001), window.BigInt(0x8000000080008008)
    ];
    ...
    function keccakF(state) {
        for (let i = 0; i < 24; i++) {  // 24 rounds in Keccak
            state = applyTheta(state);
            state = applyRho(state);
            state = applyPi(state);
            state = applyChi(state);
